...
import StackScene from '../components/three/StackScene'
import { stackMotion } from '../components/three/stackMotion'
...
export default function StudioExperience({ onBack }) {
  const [activeChapter, setActiveChapter] = useState(0)
  const [currentFilling, setCurrentFilling] = useState(DEFAULT_FILLING_ID)
  const [isSwapping, setIsSwapping] = useState(false)

  // ‏3D حقيقي عند توفر WebGL واحترام prefers-reduced-motion، وإلا الستاك القديم (CSS 3D)
  const use3D = useMemo(
    () =>
      supportsWebGL() &&
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (photoRef.current) gsap.set(photoRef.current, { autoAlpha: 0 })

      // ─── مسار الـ 3D: الـ timeline يحرّك stackMotion والمشهد يقرأها كل إطار ──
      if (use3D) {
        Object.assign(stackMotion, {
          scale: 1,
          lift: 0,
          explode: 0,
          ingredients: 0,
        ...
        mm.add(
          ...
          (self) => {
            ...
            // Discover → دنوّ بطولي
            tl.to(m, { scale: mobile ? 1.0 : 1.16, lift: 0.22, zoom: mobile ? -0.4 : 0.7, duration: 1 }, 0)
              .to(copyRefs.current[0], { autoAlpha: 0, y: -18, duration: 0.35 }, 0.72)
              .to(copyRefs.current[1], { autoAlpha: 1, y: 0, duration: 0.35 }, 0.86)
              // Explore → انفصال الطبقات
              .to(m, { explode: 1, duration: 1.25 }, 0.95)
              .to(labelRefs.current, { autoAlpha: 1, y: 0, stagger: 0.09, duration: 0.4 }, 1.25)
              // الذروة: السلطة تتفكك لمكوناتها — بندورة، جبنة، خيار، جرجير
              .to(m, { ingredients: 1, duration: 0.9, ease: 'power2.out' }, 1.45)
              .to(m, { lift: mobile ? 0.42 : 0.62, duration: 1 }, 1.85)
              .to(labelRefs.current, { autoAlpha: 0, y: -10, stagger: 0.05, duration: 0.35 }, 2.05)
              // إعادة التجميع
              .to(m, { ingredients: 0, duration: 0.65, ease: 'power2.inOut' }, 2.08)
              .to(m, { explode: 0.14, lift: 0.08, duration: 0.85 }, 2.35)
              .to(copyRefs.current[1], { autoAlpha: 0, y: -18, duration: 0.35 }, 2.45)
              .to(copyRefs.current[2], { autoAlpha: 1, y: 0, duration: 0.35 }, 2.6)
              // Create → ميلان عرض خفيف
              .to(m, { scale: mobile ? 0.94 : 1.06, tilt: -0.05, zoom: 0.3, duration: 0.8 }, 2.65)
              .to(copyRefs.current[2], { autoAlpha: 0, y: -18, duration: 0.35 }, 3.32)
              .to(copyRefs.current[3], { autoAlpha: 1, y: 0, duration: 0.35 }, 3.5)
              // Taste → الـ 3D يذوب وتحل مكانه الصورة الفوتوغرافية للنكهة المختارة
              .to(m, { explode: 0.3, scale: mobile ? 0.84 : 0.92, lift: -0.18, tilt: 0, zoom: 0, duration: 0.9 }, 3.48)
              .to('.stack-canvas', { autoAlpha: 0, duration: 0.55 }, 3.52)
              .to(photoRef.current, { autoAlpha: 1, duration: 0.7 }, 3.55)
          }
        )
        return
      }

      // ─── المسار القديم (CSS 3D) — fallback بلا WebGL أو مع reduced-motion ───
      const nodes = [
        productRef.current,
        topRef.current,
        fillingRef.current,
      ...
      mm.add(
        ...
        (self) => {
          ...
          tl.to(productRef.current, { scale: mobile ? 1.02 : 1.18, y: mobile ? -6 : -20, duration: 1 }, 0)
            ...
            .to(productRef.current, { autoAlpha: 0, duration: 0.55 }, 3.6)
            .to(photoRef.current, { autoAlpha: 1, duration: 0.7 }, 3.62)
        }
      )
    }, sceneRef)
  ...
  const chooseFilling = useCallback(
    (id) => {
      if (id === currentFilling) return
      if (use3D) {
        // المشهد ثلاثي الأبعاد يدير أنيميشن التبديل بنفسه
  ...
  return (
    <main className="premium-experience" data-chapter={chapters[activeChapter].id}>
      ...
      <section ref={sceneRef} className="cinema-scene" aria-label="The Bakery product experience">
        ...
        {use3D ? (
          <StackScene fillingImage={activeFilling.image} />
        ) : (
        ...
        {/* الختام الفوتوغرافي — صورة المنتج الحقيقي للنكهة المختارة (فصل Taste) */}
        <div ref={photoRef} className="taste-photo" aria-hidden="true">
          {fillingOptions.map((option) => (
            <img
              key={option.id}
              src={option.hero}
              alt=""
              className={option.id === currentFilling ? 'is-current' : ''}
            />
          ))}
        </div>
